...
FootageFlow bundles the platform executable from [yt-dlp 2026.07.04](https://github.com/yt-dlp/yt-dlp/releases/tag/2026.07.04) for best-effort public-media interoperability. yt-dlp is distributed under [The Unlicense](https://github.com/yt-dlp/yt-dlp/blob/master/LICENSE). Each build downloads the fixed platform release and verifies its SHA-256 before packaging.

FootageFlow also bundles separate `ffmpeg` and `ffprobe` executables for clip extraction and optional editing-compatible output. The macOS arm64 tools are built reproducibly from checksummed [FFmpeg 8.0.3](https://ffmpeg.org/releases/) and pinned [x264](https://code.videolan.org/videolan/x264) sources without any `--enable-nonfree` component. The Windows x64 tools use a checksummed, dated GPL build from [BtbN/FFmpeg-Builds](https://github.com/BtbN/FFmpeg-Builds). The applicable GPL licenses plus exact source/build references are included in the packaged `Licenses/ffmpeg` directory. FFmpeg remains a separate executable invoked through its command-line interface and does not change FootageFlow's MIT-licensed source code.
...
- Openverse public image and audio API
- Dailymotion public discovery API
