...
ffmpeg="$app/Contents/Resources/Tools/ffmpeg"
ffprobe="$app/Contents/Resources/Tools/ffprobe"
...
if strings "$ffmpeg" "$ffprobe" | LC_ALL=C grep -Eq '/Users/|/Volumes/|AIza[0-9A-Za-z_-]{20,}|sk-[A-Za-z0-9_-]{20,}|gh[pousr]_[A-Za-z0-9_]{20,}|BEGIN (RSA |OPENSSH |EC )?PRIVATE KEY'; then
  printf 'BINARY_PRIVACY_SCAN failed: personal path or credential-like data found in bundled FFmpeg tools\n' >&2
  exit 1
fi
