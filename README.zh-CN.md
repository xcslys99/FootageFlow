...
搜索结果可以切换**分组**和**全部结果**。本机分析会比较规范化来源 ID/URL、相互印证的元数据，以及有限数量的缩略图指纹；不会为比较而下载完整视频。每个重复组都保留原始卡片、操作和各自的授权信息。**推荐版本**只是方便整理，不代表版权或授权判断。可以在设置中关闭检测或默认折叠。详见 [Duplicate Review 说明](docs/DUPLICATE_REVIEW.md)（技术文档为英文）。
