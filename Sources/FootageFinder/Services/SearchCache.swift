...
actor SearchCache {
    ...
    init() {
        ...
        directory = base.appendingPathComponent("FootageFinder/SearchResults", isDirectory: true)
    ...
    func assets(provider: ProviderID, request: SearchRequest) -> [MediaAsset]? {
        ...
        guard Date().timeIntervalSince(modified) < ttl, let data = try? Data(contentsOf: url) else { return nil }
    ...
    private func fileURL(provider: ProviderID, request: SearchRequest) -> URL {
        let raw = "\(provider.rawValue)|\(request.query)|\(request.mediaType.rawValue)|\(request.orientation.rawValue)|\(request.resolution.rawValue)|\(request.duration.rawValue)"
