...
struct DownloadProgress: Identifiable {
    ...
    var message: String {
        ...
        case .downloading:
            ...
            return progress > 0 ? tr("download.downloadingPercent", Int(progress * 100)) : tr("download.downloading")
...
final class DownloadManager: NSObject, ObservableObject, URLSessionDownloadDelegate, @unchecked Sendable {
    ...
    func start(asset: MediaAsset, projectID: UUID?, projectName: String?, segmentIndex: Int? = nil) {
        if let current = states[asset.stableID], current.status == .waiting || current.status == .downloading { return }
        guard asset.downloadable, let url = try? URLValidator.remote(asset.downloadURL) else {
            states[asset.stableID] = progress(asset: asset, projectID: projectID, projectName: projectName, destination: nil, status: .failed, detailKey: "download.notAvailable")
            return
        }
        let directory = DownloadPathSafety.projectDirectory(projectName: projectName)
        do { try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true) }
        catch {
            states[asset.stableID] = progress(asset: asset, projectID: projectID, projectName: projectName, destination: directory, status: .failed, detailKey: "download.createFolderFailed")
            return
        }
        ...
        if FileManager.default.fileExists(atPath: existing.path) {
            states[asset.stableID] = progress(asset: asset, projectID: projectID, projectName: projectName, destination: existing, value: 1, status: .completed, detailKey: "download.duplicate", localURL: existing)
        ...
        lock.lock(); recoverableContexts[asset.stableID] = context; lock.unlock()
        enqueue(context, validatedURL: url)
    }

    @MainActor
    func retry(stableID: String) {
        if let current = states[stableID], current.status == .waiting || current.status == .downloading { return }
        lock.lock(); let storedContext = recoverableContexts[stableID]; lock.unlock()
        guard var context = storedContext, let url = try? URLValidator.remote(context.asset.downloadURL) else { return }
        ...
        if FileManager.default.fileExists(atPath: context.destination.path) {
            ...
            context = DownloadContext(asset: context.asset, projectID: context.projectID, projectName: context.projectName, segmentIndex: context.segmentIndex, destination: FileNameSanitizer.uniqueURL(in: directory, preferredName: FileNameSanitizer.fileName(asset: context.asset, index: context.segmentIndex)), retryCount: 0)
        }
        lock.lock(); recoverableContexts[stableID] = context; lock.unlock()
        enqueue(context, validatedURL: url)
    }

    @MainActor
    func removeState(stableID: String) {
        guard let state = states[stableID], state.status != .waiting, state.status != .downloading else { return }
        states.removeValue(forKey: stableID)
        lock.lock(); recoverableContexts.removeValue(forKey: stableID); lock.unlock()
    }
    ...
    private func enqueue(_ context: DownloadContext, validatedURL url: URL) {
        ...
        if activeCount < 3 {
            ...
            speedSamples[task.taskIdentifier] = DownloadSpeedSample(bytes: 0, time: .now, smoothedBytesPerSecond: 0)
            lock.unlock()
    ...
    nonisolated func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didWriteData bytesWritten: Int64, totalBytesWritten: Int64, totalBytesExpectedToWrite: Int64) {
        ...
        var sample = speedSamples[downloadTask.taskIdentifier] ?? DownloadSpeedSample(bytes: 0, time: now, smoothedBytesPerSecond: 0)
        let elapsed = now.timeIntervalSince(sample.time)
        if elapsed >= 0.2 {
            let instantaneous = Double(max(0, totalBytesWritten - sample.bytes)) / elapsed
            sample.smoothedBytesPerSecond = sample.smoothedBytesPerSecond > 0 ? sample.smoothedBytesPerSecond * 0.7 + instantaneous * 0.3 : instantaneous
            sample.bytes = totalBytesWritten; sample.time = now; speedSamples[downloadTask.taskIdentifier] = sample
        }
        lock.unlock()
        ...
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.states[context.asset.stableID] = self.progress(context: context, value: progress, status: .downloading, speed: sample.smoothedBytesPerSecond)
        }
    }
    ...
    nonisolated func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        do {
            ...
            do { try SourceSidecar.write(asset: context.asset, mediaURL: context.destination, projectName: context.projectName, segmentIndex: context.segmentIndex) }
            catch { try? FileManager.default.removeItem(at: context.destination); throw error }
            lock.lock(); finishedTasks.insert(downloadTask.taskIdentifier); lock.unlock()
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                self.states[context.asset.stableID] = self.progress(context: context, value: 1, status: .completed, localURL: context.destination)
            ...
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                self.states[context.asset.stableID] = self.progress(context: context, status: .failed, detailKey: "download.saveFailed")
            }
        }
    }
    ...
    nonisolated func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        if context.retryCount < 2, let url = try? URLValidator.remote(context.asset.downloadURL) {
            ...
            lock.lock(); contexts[retry.taskIdentifier] = context; recoverableContexts[context.asset.stableID] = context; speedSamples[retry.taskIdentifier] = DownloadSpeedSample(bytes: 0, time: .now, smoothedBytesPerSecond: 0); lock.unlock()
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
    ...
    nonisolated private func startNextAfterTerminalTask() {
        ...
        contexts[task.taskIdentifier] = next; recoverableContexts[next.asset.stableID] = next; speedSamples[task.taskIdentifier] = DownloadSpeedSample(bytes: 0, time: .now, smoothedBytesPerSecond: 0); activeCount += 1
        lock.unlock()
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
    ...
    private func progress(context: DownloadContext, value: Double = 0, status: DownloadStatus, speed: Double = 0, detailKey: String? = nil, localURL: URL? = nil) -> DownloadProgress {
        progress(asset: context.asset, projectID: context.projectID, projectName: context.projectName, destination: context.destination, value: value, status: status, speed: speed, retryCount: context.retryCount, detailKey: detailKey, localURL: localURL)
    }

    private func progress(asset: MediaAsset, projectID: UUID?, projectName: String?, destination: URL?, value: Double = 0, status: DownloadStatus, speed: Double = 0, retryCount: Int = 0, detailKey: String? = nil, localURL: URL? = nil) -> DownloadProgress {
        DownloadProgress(id: asset.stableID, asset: asset, projectID: projectID, projectName: projectName, destination: destination, progress: value, status: status, bytesPerSecond: speed, retryCount: retryCount, detailKey: detailKey, localURL: localURL)
    }

    private func progress(from current: DownloadProgress, status: DownloadStatus) -> DownloadProgress {
        DownloadProgress(id: current.id, asset: current.asset, projectID: current.projectID, projectName: current.projectName, destination: current.destination, progress: current.progress, status: status, bytesPerSecond: 0, retryCount: current.retryCount, detailKey: nil, localURL: current.localURL)
    }
}
