...
actor AppLogger {
  ...
  func write(
    provider: ProviderID?, requestType: String, status: Int? = nil, error: Error? = nil,
    detail: String? = nil
  ) {
    let formatter = ISO8601DateFormatter()
    let safeDetail = detail.map(Self.redact) ?? "-"
    let line =
      "\(formatter.string(from: .now)) provider=\(provider?.rawValue ?? "app") request=\(Self.redact(requestType)) status=\(status.map(String.init) ?? "-") error=\(error.map { String(describing: type(of: $0)) } ?? "-") detail=\(safeDetail)\n"
