...
struct SettingsView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                GroupBox(tr("language.menu")) {
                    Picker(tr("language.menu"), selection: Binding(get: { localization.language }, set: localization.setLanguage)) {
                ...
                GroupBox(tr("settings.noKeySources")) {
                    VStack(spacing: 0) {
                        ...
                        Divider(); noKeyRow(.internetArchive, detail: tr("settings.archiveDetail"))
                    }.padding(6)
                }
                ...
                GroupBox(tr("settings.privacy")) { Text(tr("settings.privacyBody")).font(.callout).padding(8) }
    ...
    private func keyRow(_ provider: ProviderID, key: Binding<String>, applyURL: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Button(tr("settings.testConnection")) { test(provider, key: key.wrappedValue) }.disabled(key.wrappedValue.trimmingCharacters(in: .whitespaces).isEmpty)
                Link(tr("welcome.howToApply"), destination: URL(string: applyURL)!)
            }
            HStack { Text(statuses[provider] ?? (key.wrappedValue.isEmpty ? tr("settings.notConfigured") : tr("settings.configured"))).foregroundStyle(statusColor(statuses[provider])); if let message = messages[provider] { Text(message).foregroundStyle(.secondary) } }.font(.caption)
        }.padding(.vertical, 10)
    }

    private func noKeyRow(_ provider: ProviderID, detail: String) -> some View {
        HStack { Toggle("", isOn: enabledBinding(provider)).labelsHidden(); VStack(alignment: .leading) { Text(provider.displayName).font(.headline); Text(detail).font(.caption).foregroundStyle(.secondary) }; Spacer(); Text(statuses[provider] ?? tr("settings.enabled")).foregroundStyle(statuses[provider] == tr("settings.connectionFailed") ? .red : .green); Button(tr("settings.testConnection")) { test(provider, key: "") } }.padding(.vertical, 10)
    }
    ...
    private func saveKey(_ provider: ProviderID, value: String) {
        do { try KeychainService.save(value.trimmingCharacters(in: .whitespacesAndNewlines), provider: provider); statuses[provider] = value.isEmpty ? tr("settings.notConfigured") : tr("settings.configured"); messages[provider] = tr("settings.savedKeychain") }
        catch { statuses[provider] = tr("settings.saveFailed"); messages[provider] = error.localizedDescription }
    }
    private func test(_ provider: ProviderID, key: String) {
        statuses[provider] = tr("settings.connecting"); messages[provider] = nil
        Task {
            ...
            do { try await source.testConnection(); await MainActor.run { statuses[provider] = tr("settings.connectionSuccess") } }
            catch { await MainActor.run { statuses[provider] = tr("settings.connectionFailed"); messages[provider] = error.localizedDescription } }
        }
    }
