...
struct DownloadsView: View {
    ...
    private var currentItems: [DownloadProgress] {
        downloads.states.values
            ...
            .sorted {
                let order: [DownloadStatus: Int] = [.downloading: 0, .waiting: 1, .failed: 2, .cancelled: 3, .completed: 4]
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            HStack {
                ...
                Spacer()
                Text(tr("common.itemsCount", currentItems.count + store.downloads.count)).foregroundStyle(.secondary)
            }.padding()
            Divider()
            if currentItems.isEmpty, store.downloads.isEmpty {
                ContentUnavailableView(tr("download.empty"), systemImage: "arrow.down.circle", description: Text(tr("download.emptyDescription")))
            } else {
                List {
        ...
        .alert(tr("download.deleteFailedTitle"), isPresented: Binding(get: { deletionError != nil }, set: { if !$0 { deletionError = nil } })) {
    ...
    private func currentRow(_ item: DownloadProgress) -> some View {
        HStack(spacing: 12) {
            AsyncImage(url: item.asset.thumbnailURL) { image in image.resizable().scaledToFill() } placeholder: { Color.secondary.opacity(0.1) }
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                Text([item.asset.provider.displayName, item.projectName ?? tr("common.uncategorized")].joined(separator: " · "))
                ...
                if let destination = item.destination {
                    Text(tr("download.saveLocation", destination.path)).font(.caption2).foregroundStyle(.tertiary).lineLimit(1)
                }
            }
            Spacer()
            if item.status == .downloading || item.status == .waiting {
            ...
            if let destination = item.destination {
                Button(tr("download.openFolder")) { DesktopPlatform.shared.open(destination.deletingLastPathComponent()) }
            }
    ...
    private func completedRow(_ record: DownloadRecord) -> some View {
        HStack(spacing: 12) {
            AsyncImage(url: URLValidator.remote(record.thumbnailURL)) { image in image.resizable().scaledToFill() } placeholder: { Color.secondary.opacity(0.1) }
                .frame(width: 110, height: 64).clipped().cornerRadius(6)
            VStack(alignment: .leading, spacing: 4) {
                Text(record.fileName).font(.headline).lineLimit(1)
                Text("\(ProviderID(rawValue: record.providerRaw)?.displayName ?? record.providerRaw) · \(record.downloadedAt.formatted(date: .abbreviated, time: .shortened))")
                    .font(.caption).foregroundStyle(.secondary)
                Text(record.localPath).font(.caption2).foregroundStyle(.tertiary).lineLimit(1)
            }
            Spacer()
            Button(tr("download.revealFinder")) { DesktopPlatform.shared.reveal(URL(fileURLWithPath: record.localPath)) }
            Button(tr("download.openFile")) { DesktopPlatform.shared.open(URL(fileURLWithPath: record.localPath)) }
            Button(tr("media.openSource")) { if let url = URLValidator.remote(record.sourcePageURL) { DesktopPlatform.shared.open(url) } }
            Menu {
