    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            VStack(alignment: .leading, spacing: 5) {
                ...
                LabeledContent(tr("media.specifications"), value: "\(asset.resolutionText) · \(asset.orientation.label)")
                if asset.mediaType == .video { LabeledContent(tr("media.duration"), value: asset.durationText) }
                LabeledContent(tr("media.license"), value: asset.licenseText)
                    .foregroundStyle(asset.licenseStatus == .unknown ? .orange : .secondary)
                    .help(asset.licenseStatus == .unknown ? tr("media.licenseUnavailable") : asset.licenseText)
                if let creator = asset.creator { LabeledContent(tr("media.creator"), value: creator).lineLimit(1) }
