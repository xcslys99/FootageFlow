...
enum DownloadPathSafety {
  static func projectDirectory(projectName: String?, root: URL = AppSettings.downloadRootURL) -> URL
  {
    ...
    let folder = FileNameSanitizer.sanitize(
      (fallback?.isEmpty == false ? fallback : nil) ?? tr("common.uncategorized"), maxLength: 60)
  ...
  static func isContained(_ candidate: URL, in root: URL = AppSettings.downloadRootURL) -> Bool {
    ...
    return normalizedCandidate.path.hasPrefix(
  ...
  static func relatedFiles(for mediaURL: URL) -> [URL] {
    ...
    return [
      mediaURL, base.appendingPathExtension("source.txt"),
      base.appendingPathExtension("source.json"),
    ]
  }
}
