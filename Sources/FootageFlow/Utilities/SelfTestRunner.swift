...
enum SelfTestRunner {
    @MainActor static func run() -> Int32 {
        ...
        let safetyRoot = FileManager.default.temporaryDirectory.appendingPathComponent(UUID().uuidString, isDirectory: true)
        check(DownloadPathSafety.isContained(safetyRoot.appendingPathComponent("Project/media.mp4"), in: safetyRoot), "Download path containment")
        check(!DownloadPathSafety.isContained(safetyRoot.deletingLastPathComponent().appendingPathComponent("outside.mp4"), in: safetyRoot), "Download path traversal rejection")
        check(!DownloadPathSafety.projectDirectory(projectName: "../../escape", root: safetyRoot).path.contains("../"), "Project folder sanitization")
        ...
        store.addHistory(SearchHistoryRecord(originalQuery: "test", keywords: ["test"], providers: [.wikimedia], projectID: project.id, resultCount: 1))
        store.addDownload(DownloadRecord(asset: sample, fileURL: safetyRoot.appendingPathComponent("Project/test.mp4"), projectID: project.id))
        store.deleteProject(id: project.id)
        check(store.projects.isEmpty && store.favorites.first?.projectID == nil && store.history.first?.projectID == nil && store.downloads.first?.projectID == nil, "Project delete isolation")
