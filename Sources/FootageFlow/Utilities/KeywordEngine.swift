...
enum KeywordEngine {
  ...
  static func keywords(for original: String, translated: String? = nil) -> [SearchKeyword] {
    ...
    let years =
      regex?.matches(in: compact, range: range).compactMap {
        Range($0.range, in: compact).map { String(compact[$0]) }
      } ?? []
    var mapped: [String] = []
    for (chinese, english) in phraseMap where compact.localizedCaseInsensitiveContains(chinese) {
    ...
    if !containsChinese(compact) {
      candidates.insert(compact.cleanQuery, at: 0)
    } else {
      candidates.append(compact)
    }
    return unique(candidates.map(\.cleanQuery).filter { !$0.isEmpty }).prefix(6).map {
...
extension String {
  fileprivate var cleanQuery: String {
    String(prefix(100)).replacingOccurrences(of: "\\s+", with: " ", options: .regularExpression)
