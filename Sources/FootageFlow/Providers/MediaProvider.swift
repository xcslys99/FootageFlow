...
enum ProviderUtilities {
  ...
  static func licenseStatus(name: String?, url: String? = nil) -> LicenseStatus {
    ...
    if text.contains("public domain") || text.contains("creativecommons.org/publicdomain")
      || text.contains("cc0")
    {
      return .publicDomain
    }
    ...
    if text.contains("open government licence") || text.contains("ogl 3") || text.contains("ogl v3")
    {
      return .attributionRequired
    }
  ...
  private static func versionSuffix(_ text: String) -> String {
    for version in ["4.0", "3.0", "2.5", "2.0", "1.0"] where text.contains("/\(version)/") {
      return " \(version)"
    }
    return ""
  }
